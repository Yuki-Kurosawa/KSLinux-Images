...
apt install postgresql-17 postgresql-17-debversion postgresql-client-17 -y #1>/dev/null #2>&1
echo DONE

echo -n "CONFIGURING postgresql ... "
cp /dak.sh/postgresql.conf /etc/postgresql/17/main/postgresql.conf 1>/dev/null #2>&1
cp /dak.sh/pg_hba.conf /etc/postgresql/17/main/pg_hba.conf 1>/dev/null #2>&1
echo DONE
...
symlinks python3-debian python3-debianbts python3-tabulate python3-sqlalchemy -y #1>/dev/null #2>&1
